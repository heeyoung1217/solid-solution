...
def main(reference_directory, solid_solution_directory):
    ns_generator = SolidSolnsGen(reference_directory)

    solid_solns = ns_generator.all_solid_solns

    for struc in solid_solns:
        filepath = '%s/%s_%s.cif' % (solid_solution_directory, struc.composition.reduced_formula,
            struc.get_space_group_info()[1])

        if filepath.split('/')[1] not in os.listdir(solid_solution_directory):
            struc.to(filename=filepath, fmt='cif')

if __name__ == "__main__":
    reference_directory = "References"
    solid_solution_directory = "solid_solution"
    main(reference_directory, solid_solution_directory)